...
BDG reflectivity with respect to pump2
...
def bc(y0, yL, kappa, delta_k, A1, A3, A2L_set):
    """
    ...
    delta_k : float64
        Phase mismatch which is related to the frequency difference.
    A2L_set : float64
        Power amplitudes of the pump2 at z=L
...
if __name__ == '__main__':
    ...
    A3 = sqrt(0.306)
    """
    Sweep the power of pump2 to obtain fig. 3(b)
    """
    A2_range = sqrt(np.linspace(2, 12, 100) * 10**(-3))
    rfwm_range = np.zeros(len(A2_range))
    for A2L_set, i in zip(A2_range, np.arange(len(A2_range))):
        """
        ...
        data = solve_bvp(
            ...
            lambda y0, yL: bc(y0, yL, kappa=kappa, delta_k=delta_k, A1=A1, A3=A3, A2L_set=A2L_set),
            z_span,
            dataStart.T
            )
    ...
    plt.plot(A2_range**2, rfwm_range * 10**(3), label = 'p1=110mW, p3=306mW')
    plt.legend()
