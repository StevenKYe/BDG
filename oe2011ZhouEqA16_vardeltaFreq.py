...
def dadz(z, y, eta1, eta2, delta_k):
    ...
    diff = [
        ...
        -1j * conj(eta1) * (A2 * power(A1) + A1 * A4 *
                            conj(A3) * exp(-1j * delta_k * z)),
        1j * eta2 * (A3 * power(A4) + A1 * A4 * conj(A2) * exp(-1j * delta_k * z)),
        -1j * conj(eta2) * (A4 * power(A3) + A2 * A3 *
                            conj(A1) * exp(1j * delta_k * z))
    ]
    return diff
...
if __name__ == '__main__':
    ...
    """
    length = 50  # the length of the waveguide, unit: m
    ...
    omega_diffpprobeSpan = 2 * pi * (48 * 10**9 + np.linspace(-100, 100, 200) * 10**6)
    ...
    for omega_diffpprobe, i in zip(omega_diffpprobeSpan, np.arange(len(omega_diffpprobeSpan))):
        ...
        delta_k0 = delta_k(1.45, 1.45 - 3.6 * 10**(-4), omega1, omega3)
        ...
        dataStart = odeintw(dadz, init, z_span, args=(
            eta1, eta2, delta_k0), tfirst=True)
    ...
    plt.plot((omega_diffpprobeSpan) / (2 * pi) - 48 * 10**9, rfwm_span, label='optical fiber')
    plt.legend()
