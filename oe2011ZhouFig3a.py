...
BDG reflectivity with respect to pump1
...
if __name__ == '__main__':
    ...
    # Phase mismatch which is related to the frequency difference
    delta_k = 0
    # Power amplitudes of the probe, which are held constant along the fiber
    A3 = sqrt(0.1)
    """
    Sweep the power of pump1 to obtain fig. 3(a)
    """
    A1_range = sqrt(np.linspace(50, 450, 100) * 10**(-3))
    rfwm_range = np.zeros(len(A1_range))
    for A1, i in zip(A1_range, np.arange(len(A1_range))):
        """
    ...
    plt.plot(A1_range**2, rfwm_range * 10**(3), label='P2=10mW, p3=100mW')
    plt.legend()
