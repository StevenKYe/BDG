...
FWM process with undepleted pump1 and probe waves
@author: YeK
...
if __name__ == '__main__':
    ...
    # Phase mismatch which is related to the frequency difference
    delta_k = 0
    # Power amplitudes of the pump1 and the probe, which are held constant along the fiber
    A1 = sqrt(0.2)
    A3 = sqrt(0.1)

    """
    define the power amplitudes of the pump2 and the diffracted wave and the length span of the optical
    fiber
    """
    ...
    """
    initial guessing of the pump2 and the diffracted wave at z=0
    """
    init = [
        sqrt(0.01), 0j
        ]
    ...
    #  The estimated reflectivity from equation (11)
    A2L = data.y[0, -1]
    rfwm_est = power(A1) * power(A2L) * ((exp(kappa * length * (power(A1) + power(A3))) - 1) /
                                         (power(A1) + power(A3)))**2

    print('The calculated reflectivity from equation (9) is {:.6f}'.format(abs(rfwm_cal)))
    print('The estimated reflectivity from equation (11) is {:.6f}'.format(rfwm_est))
