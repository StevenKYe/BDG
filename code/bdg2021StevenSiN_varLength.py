...
def dadz(z, y, eta1, eta2, delta_k, alpha):
    ...
    A1, A2, A3, A4 = y
    diff = [
        1j * eta1 * (A1 * power(A2) + A2 * A3 * conj(A4) * exp(1j * delta_k * z)) - alpha * A1,
        -1j * conj(eta1) * (A2 * power(A1) + A1 * A4 * conj(A3) * exp(-1j * delta_k * z)) + alpha * A2,
        1j * eta2 * (A3 * power(A4) + A1 * A4 * conj(A2) * exp(-1j * delta_k * z)) - alpha * A3,
        -1j * conj(eta2) * (A4 * power(A3) + A2 * A3 * conj(A1) * exp(1j * delta_k * z)) + alpha * A4
        ]
    return diff


def bc(y0, yL, eta1, eta2, delta_k, p1, p2, p3):
    """
    ...
    delta_k : float64
        Phase mismatch which is related to the frequency difference.
    p1 : complex128
        Optical power of the pump1.
    p2 : complex128
        Optical power of the pump2.
    p3 : complex128
        Optical power of the probe.
    ...
    """
    return [A10 - sqrt(p1), A2L - sqrt(p2), A30 - sqrt(p3), A4L]
...
def printSci(num):
    """
    print the value in scientific way
...
if __name__ == '__main__':
    ...
    """
    length = 0.5  # the length of the waveguide, unit: m
    lamb = 1550 * 10**(-9)  # wavelength at 1550 nm
    rho = 3100   # density of the silicon nitride, unit kg/m ** (-3)
    r_e = 0.174  # electrostrictive constant of the silicon nitride
    gamma_b = (2 * pi) * 153 * 10**6  # Brillouin bandwidth, unit: rad * Hz
    omega_b = (2 * pi) * 10.93 * 10**9  # Brillouin frequency, unit: rad * Hz
    A_eff = 1.85 * 10**(-12)  # acoustic-optic effective area, unit: m ** 2
    neff_te = 1.535  # effective index of the TE mode in the SDS waveguide
    neff_tm = 1.456  # effective index of the TM mode in the SDS waveguide
    p1 = 2 + 0j   # optical power of the pump1, unit: W
    p2 = 1 + 0j  # optical power of the pump2, unit: W
    p3 = 2 + 0j   # optical power of the probe, unit: W
    alpha = 1.15  # propagation loss, unit: m**(-1) 1.15
    ...
    lengthSpan = np.linspace(0.1, 1, 20)

    rfwm_span = np.zeros(len(lengthSpan))  # calculated reflectivity at different Omega
    for length, i in zip(lengthSpan, np.arange(len(lengthSpan))):
        omega3 = omega1 + delta_omega(neff_te, neff_tm, lamb)  # actual angle frequency of the probe

        #  Phase mismatching at different probe frequencies
        delta_k0 = delta_k(1.535, 1.456, omega1, omega3)

        #  Coupling coefficient between the two pumps
        eta1 = etaV2(omega_diffpump, 0.16, omega_b, gamma_b)

        #  Coupling coefficient between the probe and the diffracted wave
        eta2 = etaV2(omega_diffpump, 0.16, omega_b, gamma_b)
        ...
        """
        init = [
            sqrt(p1), sqrt(p2), sqrt(p3), 0j
            ]

        """
        initial guessing for the BVP solver
        """
        dataStart = odeintw(dadz, init, z_span, args=(eta1, eta2, delta_k0, alpha), tfirst=True)

        """
        BVP solver
        """
        data = solve_bvp(
            lambda z, y: dadz(z, y, eta1=eta1, eta2=eta2, delta_k=delta_k0, alpha=alpha),
        ...
        """
        print('Progress: {}%'.format(100 * i / len(lengthSpan)))

    plt, subfigs = plt.subplots(1,1, dpi=200, figsize=(6,3))
    subfigs.plot(lengthSpan, rfwm_span, label = 'Silicon Nitride')
    subfigs.legend()
