...
def dadz(z, y, kappa, delta_k):
    ...
    A1, A2, A3, A4 = y
    diff = [
        -kappa * (A1 * power(A2) + A2 * A3 * conj(A4) * exp(1j * delta_k * z)),
        -kappa * (A2 * power(A1) + A1 * A4 * conj(A3) * exp(-1j * delta_k * z)),
        -kappa * (A3 * power(A4) + A1 * A4 * conj(A2) * exp(-1j * delta_k * z)),
        -kappa * (A4 * power(A3) + A2 * A3 * conj(A1) * exp(1j * delta_k * z))
        ]
    return diff


def bc(y0, yL, kappa, delta_k):
    """
...
if __name__ == '__main__':
    ...
    """
    dataStart = odeintw(dadz, init, z_span, args=(kappa, delta_k), tfirst=True)

    """
    BVP solver
    """
    data = solve_bvp(
        lambda z, y: dadz(z, y, kappa=kappa, delta_k=delta_k),
        lambda y0, yL: bc(y0, yL, kappa=kappa, delta_k=delta_k),
        z_span,
        dataStart.T
        )
    ...
    print('The calculated reflectivity from equation (3) is {:.6f}'.format(rfwm_cal))
