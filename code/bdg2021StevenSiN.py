...
if __name__ == '__main__':
    ...
    p1 = 1 + 0j   # optical power of the pump1, unit: W
    ...
    omega_diffpprobeSpan = 2 * pi * np.linspace(-400, 400, 200) * 10**6 + (
        delta_omega(neff_te, neff_tm, lamb))

    rfwm_span = np.zeros(len(omega_diffpprobeSpan))  # calculated reflectivity at different Omega
    ...
    subfigs.plot((omega_diffpprobeSpan - delta_omega(neff_te, neff_tm, lamb)) / (2 * pi), rfwm_span, label = 'Ppump1=1W & Ppump2=1W')
    subfigs.legend()
